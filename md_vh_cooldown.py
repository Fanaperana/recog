...
while cap.isOpened():
    ...
    #         if cooldown_counter == 0 and is_open_palm(landmarks.landmark):
    ...
    if hand_results.multi_hand_landmarks and not countdown_active:
        for landmarks in hand_results.multi_hand_landmarks:
            if is_open_palm(landmarks.landmark):
                ...
                countdown = COUNTDOWN_TIME * frames_per_second  # Convert countdown to frames

    # Display countdown if active
    if countdown_active:
        seconds_remaining = countdown // frames_per_second
