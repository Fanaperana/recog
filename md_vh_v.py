...
def is_open_palm(landmarks):
    ...
    # Thumb
    if landmarks[mp_hands.HandLandmark.THUMB_MCP].x > landmarks[mp_hands.HandLandmark.THUMB_TIP].x:
        return False

    # Other fingers
    for finger in [mp_hands.HandLandmark.INDEX_FINGER_MCP, mp_hands.HandLandmark.MIDDLE_FINGER_MCP, 
    ...
    if landmarks[mp_hands.HandLandmark.WRIST].y < landmarks[mp_hands.HandLandmark.MIDDLE_FINGER_MCP].y:
        return False
        ...
        for landmarks in hand_results.multi_hand_landmarks:
            for landmarks in hand_results.multi_hand_landmarks:
                if is_open_palm(landmarks.landmark):
                    cv2.putText(frame, "Open Palm Detected!", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 2)
                    # # Get specific landmarks
                    # thumb_tip = landmarks.landmark[mp_hands.HandLandmark.THUMB_TIP]
                    # pinky_base = landmarks.landmark[mp_hands.HandLandmark.PINKY_MCP]

                    # # Compute distance between thumb tip and pinky base (this is just one way to check for an open palm)
                    # distance = ((thumb_tip.x - pinky_base.x) ** 2 + (thumb_tip.y - pinky_base.y) ** 2) ** 0.5

                    # if distance > 0.1:  # This threshold might need adjustment
